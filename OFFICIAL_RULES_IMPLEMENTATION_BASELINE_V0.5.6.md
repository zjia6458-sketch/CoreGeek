...
- 顶层 `prompt/executeCmd` 使用 AuxiliaryDecision 建模，不属于 RoleAction。
- Day1 夜前基础墙体目标为 8；后续目标 12/16/20，是策略基线，不属于服务器合法性规则。
- prepare 允许已邻接资源的 `collect`，但不允许重新开始远距离 ResourceApproach。
- 生产日志级别统一为 LOW/MEDIUM/HIGH，默认 MEDIUM。

---

## V0.5.6 策略配置边界

本版本把策略软参数集中到 `PolicyState.thresholds / parameters / utility_weights`，并允许 Runtime 以受限 Patch 小步更新。该机制**不改变任何官方游戏硬规则**。

采矿新增明确阶段：白天早期填包、正常当前矿黏性、建墙施工预留、最后 8 回合紧急撤离；资源选择默认距离优先于市场价值，权重均可从 `main3.py` 修改。
